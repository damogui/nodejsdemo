...
var defBeg = begMonthStr;
var defEnd = endMonthStr;
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.Data && data.Data.length > 0) {
                ...
                lui.initDropDownList({ warpid: "drop_jxd", width: 100, nameField: 'name', idField: 'id', data: arrJxd, selectedCallBack: loadSchoolChange, subtextlength: 6 });//学校和班级的联动
                ...
                lui.initDropDownList({ warpid: "dif_class", width: 80, nameField: 'name', idField: 'id', data: arrClass, selectedCallBack: loadBackDatas, subtextlength: 8 });//学校和班级的联动
                loadBackDatas();//无记录
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.Data && data.Data.length > 0) {
                ...
                lui.initDropDownList({ warpid: "dif_lesson", width: 100, nameField: 'name', idField: 'id', data: arrLesson, selectedCallBack: loadBackDatas, subtextlength: 5 });
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.Data && data.Data.length > 0) {
                ...
                lui.initDropDownList({ warpid: "dif_class", width: 80, nameField: 'name', idField: 'id', data: arrClass, selectedCallBack: loadBackDatas, subtextlength: 5 });//学校和班级的联动
...
//学校下拉时的联动
function loadSchoolChange() {

    if (userRole != "3") {//管理员


        var timeSchoolId = $("#drop_jxd").attr("data-id");//学校的下拉

        if (timeSchoolId == "0") {
            loadBackDatas();//加载数据直接

        } else {
            $.ajax({
                type: "post",
                url: "/Org/OrgStatManage/GetOrgSchoolTime",
                dataType: "json",
                data: {
                    data: timeSchoolId
                },
                success: function (data) {
                    ...
                    if (data.Data) {
                        ...
                        $("#txtBegTime").val(data.Data);//截止时间
                        loadBackDatas();//再调取后台数据
...
function loadBackDatas() {
    ...
    if (type == 3) {
        ...
        if (begTime.getTime() > endTime.getTime()) {
            ...
            //alert("开始时间不能大于结束时间");
            $("#txtBegTime").val(defBeg);
            $("#txtEndTime").val(defEnd);//赋原值
            return;
        }

        defBeg = json.BeginTime;
        defEnd = json.EndTime;//赋默认值
