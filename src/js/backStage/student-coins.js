...
function getAwardListData(schoolID) {
    $("p[data-type='edit-info']").css({ "visibility": "visible" }).html("");
    $("p[data-type='edit-info']").attr("data-ispass", 0);
    $.ajax({
        url: "/Org/Currency/GetAwardListSample",
        type: "post",
        async: true,
        data: { awardType: 0, schoolID: schoolID, awardID: 0, pageSize: 1000, pageIndex: 1, awardWays: 1 },
                ...
                drop_awardlist=  lui.initDropDownList({
                    ...
                    selectedCallBack: function (warpid, value, text, json) {
                        ...
                        if (parseInt(hcoin) < parseInt(coin)) {
                            ...
                            var h = '需要 ' + coin + ' 个学币';
                            $("p[data-type='edit-info']").attr("data-ispass", 1);
                            $("p[data-type='edit-info']").css({ "visibility": "visible" }).html(h);
                        }
                    }
...
$("#edit-ok").click(function () {
    if ($("p[data-type='edit-info']").attr("data-ispass") != 1) {
        return false;
    }
    ...
    row_data.Remark ="奖品兑换："+ j.AwardName;
